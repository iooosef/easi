      ...
      <div className="modal-header">
        <div>
          ...
          <span className="text-sm text-base-content/50">
            {vehicle.vehicleModel} · {vehicle.vehiclePlateNum}
          </span>
        </div>
      </div>
      ...
      const res = await apiFetch("/api/vehicle-logs", {
        ...
        body: JSON.stringify({
          ...
          driverEmployeeId: form.driverEmployeeId
            ? Number(form.driverEmployeeId)
            : null,
          odometerStart:
            form.odometerStart !== "" ? Number(form.odometerStart) : null,
          odometerEnd:
            form.odometerEnd !== "" ? Number(form.odometerEnd) : null,
          status: form.status,
          date: form.date || null,
        }),
      });
    ...
    <div className="modal-content w-full max-w-2xl my-auto">
      <div className="modal-header">
        <h3 className="modal-title">
          New Vehicle Log — {vehicle.vehicleModel}
        </h3>
        <button
          type="button"
        ...
        <form id="add-vehicle-log-form" onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              >
                ...
                <div className="flex gap-1 shrink-0">
                  <button
                    ...
                    onClick={() =>
                      pushModal(
                        <SchedulePickerLayer onSelect={handleScheduleSelect} />,
                      )
                    }
                  >
            ...
            <div className="sm:col-span-2 flex flex-col gap-1">
              ...
              >
                <span
                  className={`text-sm truncate ${form._driverDisplay ? "" : "text-base-content/40"}`}
                >
                ...
                <button
                  ...
                  onClick={() =>
                    pushModal(
                      <DriverPickerLayer onSelect={handleDriverSelect} />,
                    )
                  }
                >
  ...
  async function handleVehicleSelect(vehicle) {
    ...
    try {
      ...
      if (res.status === 200) {
        ...
        pushModal(
          <VehicleStillOutModal vehicle={vehicle} incompleteLog={incomplete} />,
        );
      } else {
        let prefillOdo = "";
