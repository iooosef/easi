    ...
    async function check() {
      try {
        const res = await apiFetch(
          `/api/vehicle-logs/latest-incomplete?vehiclesId=${vehiclesId}`,
        );
        if (!active) return;
        if (res.status === 200) {
    ...
    return (
      <div className="modal-content w-full max-w-sm my-auto">
        ...
        <div className="modal-body flex flex-col gap-4">
          <div className="alert alert-error py-3">
            ...
            <span className="text-sm">
              This vehicle has an ongoing trip with no end odometer recorded.
              Return the vehicle and log the end odometer before adding a new
              trip.
            </span>
          </div>
        ...
        <div className="modal-footer">
          <button type="button" className="btn btn-primary" onClick={popModal}>
            OK
          </button>
        </div>
      </div>
              ...
              pushModal(
                <DriverPickerLayer
                  onSelect={(e) => {
                    setForm((prev) => ({
                      ...prev,
                      driverEmployeeId: String(e.employeeId),
                    }));
                    setDriverLabel(
      ...
      <div className="modal-body flex flex-col gap-4">
        <div className="grid grid-cols-2 gap-x-6 gap-y-3 text-sm">
          ...
          <div className="flex flex-col gap-0.5">
            ...
            <span className="font-medium">
              {log.schedId != null ? `Sched #${log.schedId}` : "—"}
            </span>
          </div>
          <div className="flex flex-col gap-0.5">
            <span className="text-xs text-base-content/50 uppercase tracking-wide">
          ...
          <div className="flex flex-col gap-0.5">
            ...
            <span
              className={`badge badge-soft ${statusBadgeClass(log.status)} text-xs`}
            >
          ...
          <div className="flex flex-col gap-0.5">
            ...
            <span className="font-medium">
              {log.odometerEnd != null
                ? `${log.odometerEnd?.toLocaleString()} km`
                : "—"}
            </span>
          </div>
...
function UpdateLogModal({ log, onRefresh }) {
  ...
  useEffect(() => {
    apiFetch(
      `/api/vehicle-logs?vehiclesId=${log.vehiclesId}&sort=vehicleLogId,desc&size=50`,
    )
      .then((r) => (r.ok ? r.json() : Promise.reject()))
      .then((data) => {
        const prev = (data.content ?? []).find(
          (l) => l.vehicleLogId < log.vehicleLogId && l.odometerEnd != null,
        );
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      popModal();
      setTimeout(
        () => notyfSuccess(`Log #${log.vehicleLogId} updated successfully.`),
        150,
      );
      onRefresh?.();
    } catch (err) {
...
function ManageGasLogsModal({ log }) {
  ...
  useEffect(() => {
    ...
    apiFetch(`/api/vehicle-gas-logs?${params}`)
      ...
      .then((data) => {
        if (active) setGasLogs(data.content ?? []);
      })
      .catch(() => {
        if (active) setGasLogs([]);
      })
      .finally(() => {
        if (active) setGasLoading(false);
      });
    return () => {
      active = false;
    };
                    ...
                    <td>
                      <div className="flex gap-1 flex-wrap">
                        <button
                          ...
                          onClick={() =>
                            pushModal(<GasLogDetailsModal gasLog={g} />)
                          }
                        >
...
function GasLogDetailsModal({ gasLog }) {
  ...
  useEffect(() => {
    ...
    apiFetch(`/api/documents/${gasLog.docuId}/file`)
      ...
      .catch(() => {
        if (active) setDocError("Could not load document.");
      })
      .finally(() => {
        ...
        <form id="add-gas-log-form" onSubmit={handleSubmit}>
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-1">
              ...
              />
              {formError.amount && (
                <span className="helper-text">{formError.amount}</span>
              )}
            </div>
            {canManageDocs && (
              <div className="flex flex-col gap-1">
...
function ReplaceGasFileModal({ gasLog, onRefresh }) {
  ...
  async function handleSubmit(e) {
    ...
    if (!ACCEPTED_TYPES.includes(file.type)) {
      setFormError({
        file: "File must be an image (JPG/PNG/GIF/WebP) or PDF.",
      });
      return;
    }
    setFormError({});
    ...
    try {
      ...
      const updateRes = await apiFetch(
        `/api/vehicle-gas-logs/${gasLog.gasLogId}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      notyfSuccess(
        `Gas log #${gasLog.gasLogId} document replaced successfully.`,
      );
      popModal();
      onRefresh?.();
    } catch (err) {
      ...
      <div className="modal-header">
        <div>
          <h3 className="modal-title">
            {gasLog.docuId != null ? "Replace File" : "Attach File"} — Gas Log #
            {gasLog.gasLogId}
          </h3>
          <span className="text-sm text-base-content/50">
            Invoice: {gasLog.invoiceId}
...
export default function VehicleLogs() {
  ...
  useEffect(() => {
    ...
    apiFetch(`/api/vehicle-logs?${params}`)
      .then((res) => {
        if (!res.ok)
          throw new Error(`Failed to load vehicle logs (${res.status})`);
        return res.json();
      })
      .then((data) => {
        if (!active) return;
      ...
      <div className="flex items-stretch justify-between h-16 mb-6">
        <div>
          <h1 className="text-3xl font-semibold">
            Vehicle Logs — {vehicleLabel}
          </h1>
          <p className="text-base-content/60 mt-1">
            Trip records for this vehicle
          </p>
        </div>
                  ...
                  {filtered.map((l) => (
                    <tr key={l.vehicleLogId}>
                      ...
                      <td className="text-sm text-base-content/70">
                        {formatDate(l.addedOn)}
                      </td>
                      <td>
                        <span
                          className={`badge badge-soft ${statusBadgeClass(l.status)} text-xs`}
                        >
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      ...
      <div className="sm:col-span-2 flex flex-col gap-1">
        ...
        <div
          className={`input input-bordered w-full flex items-center justify-between gap-2${formError.schedId ? " is-invalid" : ""}`}
        >
          <span
            className={`text-sm truncate ${form._scheduleDisplay ? "" : "text-base-content/40"}`}
          >
        ...
        {formError.schedId && (
          <span className="helper-text">{formError.schedId}</span>
        )}
      </div>
      ...
      <div className="sm:col-span-2 flex flex-col gap-1">
        ...
        {formError.destination && (
          <span className="helper-text">{formError.destination}</span>
        )}
      </div>
      ...
      <div className="sm:col-span-2 flex flex-col gap-1">
        ...
        <div
          className={`input input-bordered w-full flex items-center justify-between gap-2${formError.driverEmployeeId ? " is-invalid" : ""}`}
        >
          <span
            className={`text-sm truncate ${driverLabel ? "" : "text-base-content/40"}`}
          >
        ...
        {formError.driverEmployeeId && (
          <span className="helper-text">{formError.driverEmployeeId}</span>
        )}
      </div>
      ...
      <div className="flex flex-col gap-1">
        ...
        <input
          ...
          onChange={onChange}
        />
        {formError.odometerStart && (
          <span className="helper-text">{formError.odometerStart}</span>
        )}
      </div>
      ...
      <div className="flex flex-col gap-1">
        ...
        <input
          ...
          onChange={onChange}
        />
        {formError.odometerEnd && (
          <span className="helper-text">{formError.odometerEnd}</span>
        )}
      </div>
