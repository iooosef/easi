...
const PROJECT_MENU_ITEMS = [
  ...
  },
  {
    ...
    roles: ["ADMIN", "ACCOUNTING", "STAFF", "CREW"],
  },
  {
    key: "ac",
    label: "Manage Air Conditioners",
    icon: "icon-[tabler--snowflake]",
    roles: null,
  },
];
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      ...
      <div className="sm:col-span-2 flex flex-col gap-1">
        ...
        />
        {formError.address && (
          <span className="helper-text">{formError.address}</span>
        )}
      </div>
      ...
      <div className="sm:col-span-2 flex flex-col gap-1">
        ...
        {formError.type && (
          <span className="helper-text">{formError.type}</span>
        )}
      </div>
      ...
      <div className="sm:col-span-2 flex flex-col gap-1">
        ...
        {formError.contactEmail && (
          <span className="helper-text">{formError.contactEmail}</span>
        )}
      </div>
      ...
      <div className="flex flex-col gap-1">
        ...
        <input
          ...
          onChange={onChange}
        />
        {formError.installationProgress && (
          <span className="helper-text">{formError.installationProgress}</span>
        )}
      </div>
      ...
      <div className="flex flex-col gap-1">
        ...
        {formError.warrantyStatus && (
          <span className="helper-text">{formError.warrantyStatus}</span>
        )}
      </div>

      <div className="flex flex-col gap-1">
        <label className="label-text font-medium">
          Warranty Date <span className="text-error">*</span>
        </label>
        <input
          type="date"
          name="warrantyDate"
          ...
          onChange={onChange}
        />
        {formError.warrantyDate && (
          <span className="helper-text">{formError.warrantyDate}</span>
        )}
      </div>
...
function ManageProjectModal({ project, onRefresh }) {
  ...
  function handleMenuSelect(key) {
    if (key === "update") {
      pushModal(
        <EditProjectModal
          ...
          onSuccess={(name) => {
            clearModals();
            setTimeout(
              () => notyfSuccess(`Project "${name}" updated successfully.`),
              150,
            );
            onRefresh();
          }}
      ...
      clearModals();
      navigate(`/service-report/project/${project.projNum}`, {
        state: { projectName: project.name },
      });
    } else if (key === "schedule") {
      clearModals();
      navigate(`/schedules/project/${project.projNum}`, {
        state: { projectName: project.name },
      });
    } else if (key === "documents") {
      clearModals();
      navigate(`/projects/${project.projNum}/documents`, {
        state: { projectName: project.name },
      });
    } else if (key === "ac") {
      clearModals();
      navigate(`/ac-units/project/${project.projNum}`, {
        state: { projectName: project.name },
      });
    }
  }
      ...
      <div className="modal-body flex flex-col gap-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-6 gap-y-4">
          ...
          <DetailField
            ...
            value={project.warrantyStatus === 1 ? "Active" : "Expired"}
          />
          <DetailField
            label="Warranty Date"
            value={formatDate(project.warrantyDate)}
          />
...
export default function Projects() {
  ...
  function openNewProject() {
    pushModal(
      <NewProjectModal
        onSuccess={(name) => {
          setTimeout(
            () => notyfSuccess(`Project "${name}" created successfully.`),
            150,
          );
          setPage(0);
          fetchProjects();
  ...
  function openManageProject(project) {
    pushModal(
      <ManageProjectModal project={project} onRefresh={fetchProjects} />,
    );
  }
      ...
      <div className="flex items-stretch justify-between h-16 mb-6">
        <div>
          ...
          <p className="text-base-content/60 mt-1">
            Manage client projects and service records
          </p>
        </div>
                    ...
                    <div className="card-body gap-2">
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <div className="flex flex-col items-end gap-1 shrink-0">
                          ...
                          <span
                            className={`badge badge-soft ${typeBadgeClass(project.type)} text-xs`}
                          >
                            {project.type}
                          </span>
                        </div>
